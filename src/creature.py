...
class Creature(Observable):
    ...
    def __str__(self):
        return str(self.information.generation)

    def get_color(self):
        return self.dna.color

    def walk(self, vertical, horizontal, delta_time):
        if(self.world.can_move((self.position[0] + vertical*delta_time, self.position[1] + horizontal*delta_time))):
            self.position = (self.position[0] + self.dna.speed_modifier*vertical*delta_time, self.position[1] + self.dna.speed_modifier*horizontal*delta_time)
